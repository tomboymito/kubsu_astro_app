...
def calculate_mass(m_k: float, m_sun: float, delta: float, r: float, f_C2: float = 1.0) -> float:
    """Расчет массы кометы по формуле: N = 10^(-0.4*(m_k - m_sun)) * delta^2 * r^2 / (1.37e-38 * f(C2))"""
    try:
...
def calculate(file_data: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        default_params = {
            ...
            "delta": 1.0,     # Геоцентрическое расстояние [а.е.]
            "r": 1.0,        # Гелиоцентрическое расстояние [а.е.]
            "f_C2": 1.0      # Поправочный коэффициент для C2
        }
