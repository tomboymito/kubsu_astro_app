...
class SublimationTab(BaseTab):
    ...
    def calculate(self):
        try:
            ...
            temperature = self.input1.text().strip()
            earth_distance = self.input2.text().strip()
            sun_distance = self.input3.text().strip()

            # Validate inputs (basic)
            if not temperature or not earth_distance or not sun_distance:
            ...
            # No file upload in this tab, so file is None
            files = {}
            data = {
