...
class SizeTab(BaseTab):
    ...
    def calculate(self):
        try:
            ...
            absolute_magnitude = self.input1.text().strip()
            albedo = self.input2.text().strip()
            angular_size = self.input3.text().strip()

            # Validate inputs (basic)
            if not absolute_magnitude or not albedo or not angular_size:
